...
X = data.drop(['Weapon Category'], axis=1)  # Drop the target column
y = data['Weapon Category']  # Target column
...
print("Shape of features after preprocessing (X):", X.shape)
...
y_resampled_df = pd.DataFrame(y_train_resampled, columns=['Weapon Category'])

# Combine features and target into a single DataFrame
resampled_data = pd.concat([X_resampled_df, y_resampled_df], axis=1)

# Save the resampled dataset to a CSV file
resampled_data.to_csv("resampled_weapon_training_data.csv", index=False)

print("Resampled data saved to 'resampled_weapon_training_data.csv'")
